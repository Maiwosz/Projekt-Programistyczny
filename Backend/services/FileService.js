...
class FileService {
    ...
    async createFile(userId, fileData, options = {}) {
        ...
        // Zapisz plik z base64 jeśli potrzeba
        if (content && !filePath) {
        ...
        const file = new File({
            user: userId,
            path: finalFilePath ? path.join(category, path.basename(finalFilePath)).replace(/\\/g, '/') : null,
            originalName,
            mimetype,
    ...
    async deleteFile(userId, fileId, permanent = false) {
        ...
        if (!file) throw new Error('Plik nie znalezony');
        
        // Oznacz do synchronizacji PRZED usunięciem
        if (file.folder) {
