...
class FolderService {
    ...
    async getFolderBySharedLink(link) {

        console.log(link.sharedLink);

        const folder = await Folder.findOne({ sharedLink: link.sharedLink });

        if (!folder) {
            throw new Error('Folder z podanym linkiem nie został znaleziony');
        }
